...
def tree1(generation: int = 5):
    tree = drawer.WMLLSystem('FX', 10, {'X': 0, 'F': 8}, {'+': 30, '-': 30}, 
                             ['X>F->F[+XFXF-X*]', 'F<X->F[-XFXF+X*]', 'F->FF'])
    tree.generate(generation)
    return tree

def tree2(generation: int = 5):
    tree = drawer.WMLLSystem('X', 6, {'X': 0, 'F': 8}, {'+': 28.5, '-': 28.5}, 
                             ['X->F-[[X]+X*]+F[+FX]-X*', 'F->FF'])
    tree.generate(generation)
    return tree

def tree3(generation: int = 5):
    tree = drawer.WMLLSystem('XF', 6, {'X': 0, 'F': 8}, {'+': 28.5, '-': 28.5}, 
...
if __name__ == '__main__':
    ...
    pen.draw_saved_trees([[530, 850], [1060, 850], [1590, 850]], drawer.FinalState.Default)
